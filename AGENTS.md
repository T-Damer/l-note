...
- Do not add a framework abstraction for a single use site.
- Keep public data contracts serializable and versioned.
- Generated packs and build output must not be edited by hand.

## Styling

- All style source lives under `styles/` as SCSS partials.
- `styles/main.scss` is the single ordered entrypoint.
- `styles.css` is generated by `npm run build:styles` and must not be edited by hand.
- The current deterministic builder supports `@use` ordering and CSS-compatible SCSS; introduce Sass-only syntax only together with an explicit Sass compiler dependency and updated lockfile.
- Keep the base palette, light theme, dark theme, semantic colors, category colors and interaction states in dedicated partials.
- Do not duplicate literal colors inside components.
- Shared animations and repeated UI patterns belong in common partials.
...
- Domain-specific query planners are optional plugins and must not leak into generic pack/search contracts.
...
- `supports`, `refines`, `contradicts` and `supersedes` are explicit links; `supersedes` changes local ranking and never deletes the reference statement.
- Every statement and relation must remain traceable to a source, note or review decision.

## Documentation

- Keep documentation short and centralized.
- `README.md` explains setup and product use.
- `docs/ARCHITECTURE.md` records the current state, architectural boundaries and invariants.
