...
export function validatePrebuiltSearchArtifacts(pack) {
  ...
  for (const [index, artifact] of pack.searchArtifacts.entries()) {
    ...
    if (!Number.isInteger(artifact.recordCount) || artifact.recordCount < 0) {
      errors.push(`${path}.recordCount must be a non-negative integer`);
    }
  }
  return errors;
}

function enabledPackRecords(packRecords) {
  return (packRecords ?? []).filter((record) => record?.enabled !== false && record?.pack);
}
...
export function selectPrebuiltSearchArtifact({
  ...
  const records = enabledPackRecords(packRecords);
